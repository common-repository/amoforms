	...
	var ColorPicker = function () {
        ...
        '</label></div><div class="colorpicker_inner"><div class="colorpicker_color"><div><div></div></div></div><div class="colorpicker_hue"><div></div></div><div class="colorpicker_new_color"></div><div class="colorpicker_current_color"></div><div class="colorpicker_hex"><label for="hex">#</label><input type="text" maxlength="6" size="6" id="hex" /></div><div class="colorpicker_rgb_r colorpicker_field"><label for="rbg_r">R</label><input type="text" maxlength="3" size="3" id="rgb_r" /><span></span></div><div class="colorpicker_rgb_g colorpicker_field"><label for="rbg_g">G</label><input type="text" maxlength="3" size="3" id="rgb_g" /><span></span></div><div class="colorpicker_rgb_b colorpicker_field"><label for="rbg_b">B</label><input type="text" maxlength="3" size="3" id="rgb_b" /><span></span></div><div class="colorpicker_hsb_h colorpicker_field"><label for="hsb_h">H</label><input type="text" maxlength="3" size="3" id="hsb_h" /><span></span></div><div class="colorpicker_hsb_s colorpicker_field"><label for="hsb_s">S</label><input type="text" maxlength="3" size="3" id="hsb_s" /><span></span></div><div class="colorpicker_hsb_b colorpicker_field"><label for="hsb_b">B</label><input type="text" maxlength="3" size="3" id="hsb_b" /><span></span></div><div class="colorpicker_submit"></div></div></div>',
			...
			setSelector = function (hsb, cal) {
				if ($(cal).data('colorpicker').isTransparent) {
					$(cal).data('colorpicker').selector.css('backgroundColor', 'transparent');
				} else {
			...
			setCurrentColor = function (hsb, cal) {
				if ($(cal).data('colorpicker').isTransparent) {
					$(cal).data('colorpicker').currentColor.css('backgroundColor', 'transparent');
				} else {
			...
			setNewColor = function (hsb, cal) {
				if ($(cal).data('colorpicker').isTransparent) {
					$(cal).data('colorpicker').newColor.css('backgroundColor', 'transparent');
				} else {
			...
			change = function (ev) {
				var cal = $(this).closest('.colorpicker'), col;
				...
				if ($(cal).data('colorpicker').isTransparent) {
					$(cal).data('colorpicker').transparencyCheckbox.prop('checked', false);
					$(cal).data('colorpicker').isTransparent = false;
				}
			...
			transparencyChange = function(ev) {
				var cal = $(this).closest('.colorpicker'), col;

				$(cal).data('colorpicker').isTransparent = $(this).prop('checked');

				if ($(cal).data('colorpicker').isTransparent) {
					col = 'transparent';

				} else {
					var hueTop = cal.find('.colorpicker_hue > div').css('top');
					hueTop = parseInt(hueTop);

					var h = 360 - (360 / 150 * hueTop)
					col = {h: h, s: 0, b: 100};
				}

				setSelector(col, cal.get(0), true);
				setHue(col, cal.get(0), true);
				setNewColor(col, cal.get(0), true);
				cal.data('colorpicker').onChange.apply(cal, [col, HSBToHex(col), HSBToRGB(col),  cal.data('colorpicker').parent ]);
        ...
        /* old code:
			...
			downIncrement = function (ev) {
				var field = $(this).parent().find('input[type="text"]').focus();
				var current = {
			...
			upIncrement = function (ev) {
				change.apply(ev.data.field.get(0), [true]);
				ev.data.el.removeClass('colorpicker_slider').find('input[type="text"]').focus();
			...
			HSBToRGB = function (hsb) {
				if(hsb == 'transparent') {return 'transparent'};
				var rgb = {};
				...
				return this.each(function () {
					if (!$(this).data('colorpickerId')) {
						...
						options.fields = cal
							...
							.on('blur', blur)
							.on('focus', focus);
						options.transparencyCheckbox = cal
							.find('#transparency_checkbox')
							.on('change', transparencyChange);

						if (opt.isTransparent) {
							options.transparencyCheckbox.prop('checked', true)
						}
